...
def experiment(model, data, loader=None, test_loader=None):
    optimizer = build_optimizer(model, lr=0.01, weight_decay=5e-4)
    if loader:
...
def per_dataset(dataset, n_trials=10): # Set default n_trials back to 10 for testing
    num_features = dataset.num_features
    num_classes = dataset.num_classes

    ds_name = getattr(dataset, 'name', dataset.__class__.__name__)

    if not os.path.exists('result/real_1'):
        os.makedirs('result/real_1')
    result_path = f'result/real_1/Ours_vs_Oversmoothing_Baselines_{ds_name}_data.npz'
    ...
    fig.savefig(f'result/real_1/Ours_vs_Oversmoothing_Baselines_{ds_name}_layers.pdf', bbox_inches='tight', pad_inches=0.2)
