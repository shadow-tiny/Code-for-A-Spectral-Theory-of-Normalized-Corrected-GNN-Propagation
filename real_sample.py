...
def train_minibatch(model, optimizer, loader, epochs=10):
    model.to(device)
    model.train()
    # For million-node graphs, 10 epochs are sufficient for convergence after sampling only training nodes
    for epoch in range(epochs):
        for data in loader:
...
def extract_balanced_two_class_subset(data):
    ...
    # Get indices of nodes in these two classes
    idx1 = torch.where(y == class1)[0]
    idx2 = torch.where(y == class2)[0]
    
    # Randomly downsample the majority class to perfectly match the size of the minority class
    ...
    # Relabel y to 0 and 1 for the evaluated nodes
    new_y_relabeled = torch.zeros_like(data.y)
    new_y_relabeled[idx1_sampled] = 0
    new_y_relabeled[idx2_sampled] = 1
    
    # Create new train/val/test split (60/20/20) restricted to the 1:1 balanced subset
    ...
    # We DO NOT extract a subgraph. We keep the entire graph topology intact
    # to preserve connectivity and global steady-state distribution,
    # but we only evaluate loss/accuracy on the balanced 1:1 masks.
    new_data = data.clone()
...
def per_dataset(dataset, n_trials=10):
    ...
    # Extract the balanced two-cluster subset
    balanced_data = extract_balanced_two_class_subset(dataset._data)
    num_classes = 2 # We ensured it has 2 classes
    ...
    import os
    if not os.path.exists('result/real_sample'):
        os.makedirs('result/real_sample')
    result_path = f'result/real_sample/GCN_vs_GCNCorrected_{ds_name}_data.npz'
    ...
    if use_loader:
        ...
        data = balanced_data # Keep on CPU
    ...
    # Change the title to match the context in the paper (Evaluating the Two-Cluster Balanced Sampling strategy)
    ...
    fig.savefig(f'result/real_sample/GCN_vs_GCNCorrected_{ds_name}_layers.pdf', bbox_inches='tight', pad_inches=0.2)
